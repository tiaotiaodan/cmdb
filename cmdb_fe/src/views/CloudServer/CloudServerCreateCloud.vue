<template>
  <el-dialog :model-value="visible" width="35%" @close="dialogClose">
    <!--标题-->
    <template #header>
      <div style="font-size: 18px; color: #409eff; font-weight: bold">导入云主机</div>
    </template>
      ...
      <div v-show="active == 1">
        <el-form-item prop="cloud" label-width="0px" style="margin-left: 25%">
          <el-radio-group v-model="form.cloud">
            ...
            <el-radio label="tencent"><img src="../../assets/img/tencend.png" /></el-radio>
          </el-radio-group>
        </el-form-item>
      </div>
      <!--第二步-->
      ...
      <div v-show="active == 3">
        <el-form-item label="选择区域：" prop="region">
          <el-select class="m-2" v-model="form.region" @click="getCloudRegion" placeholder="请选择" style="width: 100%">
            <el-option v-for="row in cloudRegion" :key="row.region_id" :label="row.region_name" :value="row.region_id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="选择分组：" prop="server_group">
          <el-select class="m-2" v-model="form.server_group" @click="getServerGroup" placeholder="请选择" style="width: 100%">
...
export default {
  ...
  data() {
    return {
      ...
      formRules: {
        ...
        region: [{ required: true, message: '请选择IDC机房', trigger: 'change' }],
  ...
  methods: {
      ...
      this.$refs.formRef.validate(valid => {
        if (valid) {
          if (this.form.cloud == 'aliyun') {
            this.$http.post('cmdb/aliyun_cloud/', this.form).then(res => {
    ...
    getCloudRegion() {
      if (this.form.cloud == 'aliyun') {
        // 携带密钥请求
        this.$http.get('cmdb/aliyun_cloud/', { params: this.form }).then(res => {
          this.cloudRegion = res.data.data
        })
      } else if (this.form.cloud == 'tencent') {
        this.$http.get('cmdb/tencent_cloud/', { params: this.form }).then(res => {
          this.cloudRegion = res.data.data
        })
      } else {
