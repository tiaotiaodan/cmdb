<template>
  <el-card class="box-card">
    ...
    <div style="margin-bottom: 10px; display: flex; align-items: center; justify-content: space-between">
      ...
      <div>
        ...
        <el-dropdown style="margin-right: 12px">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item @click="handelVmServerCreate">
                <el-icon color="#409EFC" :size="20"><Edit /></el-icon>
                新建单台主机
              </el-dropdown-item>
              <el-dropdown-item @click="handelVmServerCreateExcel">
        ...
        <el-popover placement="left" :width="100" v-model:visible="columnVisible">
          ...
          <el-checkbox v-model="showColumn.vm_host">虚拟主机</el-checkbox>
    ...
    <div>
      <!--数据表格-->
      <el-table :data="VmServerData" border style="width: 100%" :header-cell-style="{ backgroundColor: '#409EFF', color: '#fff', fontsize: '14px' }">
        ...
        <el-table-column prop="vm_host.ssh_ip" label="虚拟主机"  width="140" sortable v-if="showColumn.vm_host" />
  ...
  <!--使用子组件-->
  <VmServerEdit v-model:visible="dialogVmServerEdit" :row="row"></VmServerEdit>
...
<script>
// 导入子组件VmServer编辑
import VmServerEdit from './VmServerEdit.vue'
...
export default {
  ...
  data() {
    return {
      ...
      serverGroup: '',
      // =============================== 编辑配置 ===============
      dialogVmServerEdit: false,
      row: '',

      // ============================== 创建 ===================
      dialogVmServerCreate: false,
      
      // ============================== Excel导入 ===================
      dialogVmServerCreateExcel: false,
  ...
  methods: {
    getallVmServer() {
      ...
      this.$http.get('cmdb/vm_server/', { params: this.urlParams }).then(res => {
        if (res.data.code == 200) {
          // 把获取数据重新赋值到表格
          this.VmServerData = res.data.data
          // 把获取的数据总量重新赋值给total
    ...
    // 新建VmServer机房
    handelVmServerCreate() {
      // 重新赋值允许打开编辑弹出框
      this.dialogVmServerCreate = true
    },
    
    // excel导入主机
    handelVmServerCreateExcel() {
      // 重新赋值允许打开创建弹出框
      this.dialogVmServerCreateExcel = true
    },
    // 编辑进行数据重新赋值
    ...
    handelVmServerSync(index, row) {
      ...
      if(row.credential) {
        this.$http
          .get('cmdb/vm_server_host_collect/', {params: {'hostname': row.hostname}})
          .then(res => {
            if(res.data.code == 200) {
        ...
        // 重新赋值允许打开同步弹出框
        this.dialogVmServerSync = true
        this.row = row
      }
      
    },
    // 删除单条数据
    handelVmServerDelete(index,row) {
      this.$confirm('你确定要删除选中的吗？', '提示', {
        ...
        this.$http
          .delete('cmdb/vm_server/' + row.id + '/')
          .then(res => {
            if (res.data.code == 200) {
              this.$message.success('删除成功')
              this.VmServerData.splice(index,1) // 根据表格索引临时删除数据
            }
          })
      }).catch(() => {
        this.$message({
